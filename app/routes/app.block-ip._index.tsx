...
export default function BlockIPPage() {
    ...
    const filteredIPs = blockedIPs.filter(
        (item) => item.ip.includes(search) || item.reason.toLowerCase().includes(search.toLowerCase())
    );

    const handleBulkRemove = useCallback(() => {
        setBlockedIPs((prev) => prev.filter((item) => !selectedItems.includes(item.id)));
        setSelectedItems([]);
    }, [selectedItems]);
    ...
    return (
        <Page title="Blocked IP Rules" primaryAction={{ content: 'Add Block Rule', onAction: handleAdd }}>
            ...
            {filteredIPs.length === 0 ? (
                ...
                <LegacyCard>
                    <ResourceList
                        ...
                        onSelectionChange={(selected) => setSelectedItems(Array.isArray(selected) ? selected : [])}
                        bulkActions={[
                            {
